...
from VideoEvents import model, controller
import VideoEvents.model as mdl
import random
...
def showVideoAlert(layout, info="Video Error", defaultResponse="There is no video resource to analyze"):
...
class VideoEventsMenu(QtGui.QWidget):
    ...
    def serializeAction(self):
        ...
        if(len(self.models_list)==0):
            return
        filename = QtGui.QFileDialog.getSaveFileName(self, "Save File", os.path.expanduser(os.getcwd()))
        ...
        with open(filename, 'w') as outfile:
            ...
            # strx = json.dumps( [ob.__dict__ for ob in self.models_list] , sort_keys=True, indent=4)
            outfile.write(strx)
    ...
    def setres(self):
        ...
        print('set res')
        dir = os.getcwd()
        fnamedir = QtGui.QFileDialog.getOpenFileName(self, 'Open file', dir)
    ...
    def addVideoEvent(self):
        ...
        model = mdl.VideoEventModel(id,self.starttime, self.stoptime, self.posx, self.posy, type, str(self.resname))
...
class Player(QtGui.QMainWindow):
    ...
    def OpenFile(self, filename=None):
        ...
        if filename is None:
            dir = os.getcwd()
            filename = QtGui.QFileDialog.getOpenFileName(self, "Open File", os.path.expanduser(dir))
        if not filename:
            return
